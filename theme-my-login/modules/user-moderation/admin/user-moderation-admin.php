...
class Theme_My_Login_User_Moderation_Admin extends Theme_My_Login_Abstract {
	...
	public function settings_field_moderation_type() {
		...
		<input name="<?php echo $this->options_key; ?>[type]" type="radio" id="<?php echo $this->options_key; ?>_type_both" value="both" <?php checked( $this->get_option( 'type' ), 'both' ); ?> />
		<label for="<?php echo $this->options_key; ?>_type_both"><?php _e( 'Admin & User Both Approval', 'theme-my-login' ); ?></label>
		<p class="description"><?php _e( 'Check this option to require new users to be approved by an administrator before they may log in.', 'theme-my-login' ); ?></p>
		<?php
	}
	...
	public function save_settings( $settings ) {
		return array(
			'type' => in_array( $settings['type'], array( 'none', 'email', 'admin', 'both' ) ) ? $settings['type'] : 'none'
		);
	}
			...
			if ( in_array( 'pending', (array) $user_object->roles ) ) {
				switch ( $this->get_option( 'type' ) ) {
					...
					case 'admin' :
						...
						break;
					case 'both' : //cbxmod
							$user_activation_key = $wpdb->get_var( $wpdb->prepare( "SELECT user_activation_key FROM $wpdb->users WHERE ID = %s", $user_object->ID ) );
	...
	public static function approve_user( $user_id ) {
		...
		$user_registerd_as = get_user_meta( $user->ID, 'user_registerd_as', true ); //cbxmod
		if(!empty($user_registerd_as)){
			$approval_role =  $user_registerd_as; //if meta exists then we set the role as user choose while registration
		}
