...
class Theme_My_Login_User_Moderation extends Theme_My_Login_Abstract {
	...
	protected function load() {
		...
		if ( in_array( $this->get_option( 'type' ), array( 'admin', 'email', 'both' ) ) ) {  //cbxmod
			...
			if ( 'email' == $this->get_option( 'type' ) || 'both' == $this->get_option( 'type' ) ) {  //cbxmod
	...
	*/
	...
	public function registration_redirect( $redirect_to ) {
		...
		switch ( $this->get_option( 'type' ) ) {
			...
			case 'admin' :
				...
				break;
			//for both at first user activtion and then admin approval
			case 'both' : //cbxmod
				$redirect_to = add_query_arg( 'pending', 'activation', $redirect_to );
				break;
		}
	...
	public function authenticate( $user, $username, $password ) {
		...
		if ( $userdata = get_user_by( 'login', $username ) ) {
			if ( array_key_exists( 'pending', (array) $userdata->$cap_key ) ) {
				if ( 'email' == $this->get_option( 'type' ) ) {  //cbxmod
				...
				else if ( 'both' == $this->get_option( 'type' ) ) {
					$user_activation_key = $wpdb->get_var( $wpdb->prepare( "SELECT user_activation_key FROM $wpdb->users WHERE ID = %s", $user->ID ) );
					if($user_activation_key != ''){
						...
						return new WP_Error( 'pending', __( '<strong>ERROR</strong>: You have activated but your registration has not yet been approved by admin. Please wait.', 'theme-my-login' ) );
					}
				}
				else
				{
	...
	public function action_messages( &$theme_my_login ) {
		...
		if ( isset( $_GET['activation'] ) ) {
			switch ( $_GET['activation'] ) {
				case 'complete' :
					if('both' === $this->get_option( 'type' )){
						if ( class_exists( 'Theme_My_Login_Custom_Passwords' ) )
							$theme_my_login->errors->add( 'activation_complete', __( 'Your email has been verified but you must now be approved by an administrator before you can log in. You will be notified by e-mail once your account has been reviewed.', 'theme-my-login' ), 'message' );
						else
							$theme_my_login->errors->add( 'activation_complete', __( 'Your email has been verified but you must now be approved by an administrator before you can log in. You will be notified by e-mail once your account has been reviewed.', 'theme-my-login' ), 'message' );
	...
	public function moderate_user( $user_id ) {
		...
		elseif ( 'both' == $this->get_option( 'type' ) ) { //cbxmod
			//check if the user verified itself
			//if not verified then send activation email

			/*
			// Validate activation key
			$user_activation_key = $wpdb->get_var( $wpdb->prepare( "SELECT user_activation_key FROM $wpdb->users WHERE ID = %s", $user_id ) );
			if($user_activation_key == ''){
			...
			*/
			self::new_user_activation_notification( $user_id );
			 ...
			 //if verified then  send  email to admin to verify the user
/*
			$user = new WP_User( $user_id );
	...
	public static function activate_new_user( $key, $login ) {
		...
		$class_ref = self::get_object();
		$type = $class_ref->get_option( 'type', 'none' );
		...
		//cbxmode
		//need to check if type == both role will not be changed  here, role should be changed when admin approve finally
		//else change
		...
		/*if('email' == self::get_option( 'type' ) || 'both' == self::get_option( 'type' )){
			//need to put role from meta field
			$user_registerd_as = get_user_meta( $user->ID, 'user_registerd_as', true );
			if(!empty($user_registerd_as)){
				$user_object->set_role( $user_registerd_as );
			}
			else{
		...
		//var_dump('fine re vai fine2'); die();

		//this will happen if only email method is used , because if admin or both method is used actual role change will be done in admin approval end
		if('email' == $type){
			//$user_object->set_role( get_option( 'default_role' ) );
	...
	public function user_activation() {
		...
		if ( ! is_wp_error( $errors ) ){
			if('both' == $this->get_option( 'type', 'none' )) {

				$user = $wpdb->get_row($wpdb->prepare("SELECT * FROM $wpdb->users WHERE user_login = %s", $_GET['login']));

				// Send approval e-mail
				if (apply_filters('send_new_user_approval_admin_notification', true))
					self::new_user_approval_admin_notification($user->ID);
			}
	...
	public static function new_user_approval_admin_notification( $user_id ) {
		...
		$message .= admin_url( 'users.php?role=pending&s='.$user_id );
